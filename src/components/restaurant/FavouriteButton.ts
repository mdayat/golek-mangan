...
const HeartFillIcon = () => {
  const heartFillSvg = document.createElementNS(SVG_NAMESPACE, "svg");
  heartFillSvg.setAttribute("xmlns", SVG_NAMESPACE);
  heartFillSvg.setAttribute("viewBox", "0 0 512 512");

  const heartFillPath = document.createElementNS(SVG_NAMESPACE, "path");
  heartFillPath.setAttribute(
    "d",
  ...
  heartFillSvg.appendChild(heartFillPath);

  return heartFillSvg;
};

const HeartUnfillIcon = () => {
  const heartUnfillSvg = document.createElementNS(SVG_NAMESPACE, "svg");
  heartUnfillSvg.setAttribute("xmlns", SVG_NAMESPACE);
  heartUnfillSvg.setAttribute("viewBox", "0 0 512 512");

  const heartUnfillPath = document.createElementNS(SVG_NAMESPACE, "path");
  heartUnfillPath.setAttribute(
    "d",
  ...
  heartUnfillSvg.appendChild(heartUnfillPath);

  return heartUnfillSvg;
};

const favouriteButtonFunctionalities = (
  favouriteButton: HTMLButtonElement,
  restaurant: Restaurant
) => {
  getFavouriteRestaurant(restaurant.id)
    .then((favouritedRestaurant: Restaurant | undefined) => {
      if (favouritedRestaurant === undefined) {
        ...
        favouriteButton.insertAdjacentText(
          "beforeend",
  ...
  favouriteButton.addEventListener("click", async () => {
    const favouritedRestaurant: Restaurant | undefined =
      await getFavouriteRestaurant(restaurant.id);
...
const FavouriteButton = (restaurant: Restaurant): HTMLButtonElement => {
